...
template<
    ...
    typename = std::enable_if_t<
        std::is_same_v<typename BucketContainer::iterator, decltype(std::begin(std::declval<std::add_lvalue_reference_t<BucketContainer>>()))> &&
        std::is_same_v<typename BucketContainer::iterator, decltype(std::end(std::declval<std::add_lvalue_reference_t<BucketContainer>>()))> &&
        std::is_same_v<void, decltype(std::declval<BucketContainer>().push_front(std::declval<std::pair<const Key, Value>>()))> &&
        ...
        std::is_same_v<bool, decltype(std::declval<BucketContainer>().empty())> &&
        std::is_same_v<std::size_t, decltype(std::declval<BucketContainer>().size())> &&
        std::is_base_of_v<std::bidirectional_iterator_tag, typename std::iterator_traits<typename BucketContainer::iterator>::iterator_category>>,
...
public:
    ...
    private:
        explicit Iterator(hash_table& hasht, table_iterator table_iter, bucket_iterator bucket_iter);
...
private:
    ...
    auto optimization_after_find(BucketContainer& table_pos, bucket_iterator bucket_pos) noexcept
        -> bucket_iterator;
...
template<typename Key, typename Value, std::size_t N, typename HashFunc, typename BucketContainer, typename KeyEqual, typename A, typename B, typename C, typename D, typename E>
auto hash_table<Key, Value, N, HashFunc, BucketContainer, KeyEqual, A, B, C, D, E>::insert(
    const value_type& value) -> std::pair<bool, hash_table::iterator> {
    auto [is_found, table_iter, bucket_iter] = find_value(value.first);
    if (!is_found) {
...
template<typename Key, typename Value, std::size_t N, typename HashFunc, typename BucketContainer, typename KeyEqual, typename A, typename B, typename C, typename D, typename E>
auto hash_table<Key, Value, N, HashFunc, BucketContainer, KeyEqual, A, B, C, D, E>::erase(const Key& key)
    -> void {
    auto [is_found, table_iter, bucket_iter] = find_value(key);
    if (is_found) {
...
template<typename Key, typename Value, std::size_t N, typename HashFunc, typename BucketContainer, typename KeyEqual, typename A, typename B, typename C, typename D, typename E>
auto hash_table<Key, Value, N, HashFunc, BucketContainer, KeyEqual, A, B, C, D, E>::erase(const_iterator pos)
    -> iterator {
...
template<typename Key, typename Value, std::size_t N, typename HashFunc, typename BucketContainer, typename KeyEqual, typename A, typename B, typename C, typename D, typename E>
auto hash_table<Key, Value, N, HashFunc, BucketContainer, KeyEqual, A, B, C, D, E>::find(const Key& key)
    -> iterator {
    auto [is_found, table_iter, bucket_iter] = find_value(key);
    if (!is_found) {
        return end();
...
template<typename Key, typename Value, std::size_t N, typename HashFunc, typename BucketContainer, typename KeyEqual, typename A, typename B, typename C, typename D, typename E>
auto hash_table<Key, Value, N, HashFunc, BucketContainer, KeyEqual, A, B, C, D, E>::find(const Key& key) const
    -> const_iterator {
    return (const_cast<hash_table*>(this))->find(key);
}

template<typename Key, typename Value, std::size_t N, typename HashFunc, typename BucketContainer, typename KeyEqual, typename A, typename B, typename C, typename D, typename E>
auto hash_table<Key, Value, N, HashFunc, BucketContainer, KeyEqual, A, B, C, D, E>::operator[](const Key& key)
    -> Value& {
    auto [is_found, table_iter, bucket_iter] = find_value(key);
    if (!is_found) {
...
template<typename Key, typename Value, std::size_t N, typename HashFunc, typename BucketContainer, typename KeyEqual, typename A, typename B, typename C, typename D, typename E>
auto hash_table<Key, Value, N, HashFunc, BucketContainer, KeyEqual, A, B, C, D, E>::empty() const
    noexcept -> bool {
    return std::all_of(m_table.begin(), m_table.end(), std::mem_fn(&BucketContainer::empty));
}

template<typename Key, typename Value, std::size_t N, typename HashFunc, typename BucketContainer, typename KeyEqual, typename A, typename B, typename C, typename D, typename E>
auto hash_table<Key, Value, N, HashFunc, BucketContainer, KeyEqual, A, B, C, D, E>::size() const
    noexcept -> std::size_t {
    auto sum_buckets = [](std::size_t curr_sum, const BucketContainer& bucket) {
...
template<typename Key, typename Value, std::size_t N, typename HashFunc, typename BucketContainer, typename KeyEqual, typename A, typename B, typename C, typename D, typename E>
auto hash_table<Key, Value, N, HashFunc, BucketContainer, KeyEqual, A, B, C, D, E>::begin() noexcept
    -> iterator {
...
template<typename Key, typename Value, std::size_t N, typename HashFunc, typename BucketContainer, typename KeyEqual, typename A, typename B, typename C, typename D, typename E>
auto hash_table<Key, Value, N, HashFunc, BucketContainer, KeyEqual, A, B, C, D, E>::begin() const
    noexcept -> const_iterator {
    return cbegin();
}

template<typename Key, typename Value, std::size_t N, typename HashFunc, typename BucketContainer, typename KeyEqual, typename A, typename B, typename C, typename D, typename E>
auto hash_table<Key, Value, N, HashFunc, BucketContainer, KeyEqual, A, B, C, D, E>::cbegin() const
    noexcept -> const_iterator {
    return (const_cast<hash_table*>(this))->begin();
}

template<typename Key, typename Value, std::size_t N, typename HashFunc, typename BucketContainer, typename KeyEqual, typename A, typename B, typename C, typename D, typename E>
auto hash_table<Key, Value, N, HashFunc, BucketContainer, KeyEqual, A, B, C, D, E>::end() noexcept
    -> iterator {
...
template<typename Key, typename Value, std::size_t N, typename HashFunc, typename BucketContainer, typename KeyEqual, typename A, typename B, typename C, typename D, typename E>
auto hash_table<Key, Value, N, HashFunc, BucketContainer, KeyEqual, A, B, C, D, E>::end() const
    noexcept -> const_iterator {
    return cend();
}

template<typename Key, typename Value, std::size_t N, typename HashFunc, typename BucketContainer, typename KeyEqual, typename A, typename B, typename C, typename D, typename E>
auto hash_table<Key, Value, N, HashFunc, BucketContainer, KeyEqual, A, B, C, D, E>::cend() const
    noexcept -> const_iterator {
    return (const_cast<hash_table*>(this))->end();
}

template<typename Key, typename Value, std::size_t N, typename HashFunc, typename BucketContainer, typename KeyEqual, typename A, typename B, typename C, typename D, typename E>
auto hash_table<Key, Value, N, HashFunc, BucketContainer, KeyEqual, A, B, C, D, E>::get_table_iter(
    const Key& key) -> table_iterator {
    return m_table.begin() + (HashFunc()(key) % N);
}

template<typename Key, typename Value, std::size_t N, typename HashFunc, typename BucketContainer, typename KeyEqual, typename A, typename B, typename C, typename D, typename E>
auto hash_table<Key, Value, N, HashFunc, BucketContainer, KeyEqual, A, B, C, D, E>::find_value(const Key& key)
    -> found_value {
    auto table_iter = get_table_iter(key);
    auto is_equal_to_key = [&key](const value_type& v) {
        return KeyEqual()(key, v.first);
    };
    auto found_bucket_iter =
        std::find_if(std::begin(*table_iter), std::end(*table_iter), is_equal_to_key);
    return found_value(found_bucket_iter != std::end(*table_iter), table_iter, found_bucket_iter);
}

template<typename Key, typename Value, std::size_t N, typename HashFunc, typename BucketContainer, typename KeyEqual, typename A, typename B, typename C, typename D, typename E>
auto hash_table<Key, Value, N, HashFunc, BucketContainer, KeyEqual, A, B, C, D, E>::optimization_after_find(
    BucketContainer& table_pos, bucket_iterator found_bucket_iter) noexcept
    -> bucket_iterator {
    try {
...
template<typename Key, typename Value, std::size_t N, typename HashFunc, typename BucketContainer, typename KeyEqual, typename A, typename B, typename C, typename D, typename E>
auto hash_table<Key, Value, N, HashFunc, BucketContainer, KeyEqual, A, B, C, D, E>::get_dummy_table_iterator()
    -> table_iterator {
    return std::prev(m_table.end());
}

template<typename Key, typename Value, std::size_t N, typename HashFunc, typename BucketContainer, typename KeyEqual, typename A, typename B, typename C, typename D, typename E>
auto hash_table<Key, Value, N, HashFunc, BucketContainer, KeyEqual, A, B, C, D, E>::get_next_non_empty_bucket(
    table_iterator pos) -> table_iterator {
    return std::find_if_not(
        pos, get_dummy_table_iterator(), std::mem_fn(&BucketContainer::empty));
}

template<typename Key, typename Value, std::size_t N, typename HashFunc, typename BucketContainer, typename KeyEqual, typename A, typename B, typename C, typename D, typename E>
template<bool IsConst>
hash_table<Key, Value, N, HashFunc, BucketContainer, KeyEqual, A, B, C, D, E>::Iterator<IsConst>::Iterator(
    hash_table& hasht, table_iterator table_iter, bucket_iterator bucket_iter)
  : m_hasht(&hasht), m_table_iter(table_iter), m_bucket_iter(bucket_iter) {
}
...
template<bool IsConst>
template<bool IsConst_, typename>
auto hash_table<Key, Value, N, HashFunc, BucketContainer, KeyEqual, A, B, C, D, E>::Iterator<IsConst>::operator=(
    const Iterator<false>& other) -> Iterator& {
    m_hasht = other.m_hasht;
    m_table_iter = other.m_table_iter;
    m_bucket_iter = other.m_bucket_iter;
...
template<typename Key, typename Value, std::size_t N, typename HashFunc, typename BucketContainer, typename KeyEqual, typename A, typename B, typename C, typename D, typename E>
template<bool IsConst>
auto hash_table<Key, Value, N, HashFunc, BucketContainer, KeyEqual, A, B, C, D, E>::Iterator<IsConst>::operator++(int)
    -> const Iterator {
    auto old_val = *this;
    ++(*this);
    return old_val;
